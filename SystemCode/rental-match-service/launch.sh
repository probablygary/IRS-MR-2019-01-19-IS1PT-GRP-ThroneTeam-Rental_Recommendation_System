...
if [ "$MVN_ARG_LINE" != "" ] ; then
    ...
    "$mvnBin" -v
    echo
    projects=( "*-model" "*-kjar" "rental-match-service")
...
if [[ "$@" =~ "docker" ]]; then
    ...
    docker run -d -p 8090:8090 --name rental-match-service apps/rental-match-service:1.0-SNAPSHOT
elif [[ "$@" =~ "openshift" ]]; then
    echo "Launching the application on OpenShift..."
    
    oc new-app rental-match-service:1.0-SNAPSHOT
    oc expose svc/rental-match-service
else

	echo "Launching the application locally..."
	pattern="rental-match-service"
	files=( $pattern )
	cd ${files[0]}
