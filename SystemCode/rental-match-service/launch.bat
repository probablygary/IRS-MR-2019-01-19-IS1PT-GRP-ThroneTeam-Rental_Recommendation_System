...
if "%*" == "" (
	...
	for %%s in ("-model" "-kjar" "rental-match-service") do (
...
:startapp
	if not x%mavenInput:docker=%==x%mavenInput% (
		...
		call docker run -d -p 8090:8090 --name rental-match-service apps/rental-match-service:1.0-SNAPSHOT
	) else if not x%mavenInput:openshift=%==x%mavenInput% (
		echo Launching the application on OpenShift...
		call oc new-app rental-match-service:1.0-SNAPSHOT
		call oc expose svc/rental-match-service
	) else (
		echo "Launching the application locally..."
		cd rental-match-service
		cd target
